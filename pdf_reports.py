...
from calendar_config import MONTH_NAMES, QUARTER_MONTHS, get_period_months, get_ytd_months
...
from statement_builder import build_pl_rows, build_partida_lookup, bs_summary
...
def _merge_current_prev(current, prev, join_key, col_names, keep_months, output_cols):
    ...
    return merged[output_cols] if not merged.empty else merged
...
def bs_summary_pdf(df_bs_current, df_bs_prev, year, period_type, period_num,
    ...
    """
    from calendar_config import MONTH_NAMES, get_end_month
...
def bs_relacionadas_nit_pdf(df_bs, period_type, period_num, partida_fn):
    ...
    if df_bs.empty:
        return pd.DataFrame()
    from calendar_config import get_end_month
    end_month = get_end_month(period_type, period_num)
