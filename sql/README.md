...
| `VISTA_BS_SUMARIO.sql` | Phase C — pre-aggregated BS summary on top of `VISTA_BS_PREPARADO_CUMSUM`. Applies the three reclassification rules (predicate uses each row's own per-month cumsum — displayed-month semantics) + native-section sign flip, then groups to partida-grain. ~30 rows × 12 months per `(CIA, YEAR)`. |
