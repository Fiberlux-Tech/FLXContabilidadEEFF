...
from models import PdfReportData
...
from calendar_config import MONTH_NAMES, QUARTER_MONTHS, MONTH_NAMES_ES, get_end_month
from company_config import COMPANY_META
...
from pdf_table_renderer import _render_table, _compute_widths, _truncate_text
...
def _assign_group_order(df, group_table, classify_fn):
    ...
    *classify_fn* maps a CUENTA_CONTABLE string to a group label (or None).
    ...
    df["_group_order"] = df["_group"].map(group_order).fillna(999).astype(int)
...
def _inject_efectivo_groups(df: pd.DataFrame, value_cols: list[str]) -> pd.DataFrame:
    ...
    """
    # Separate the TOTAL row from data rows
    is_total = df["CUENTA_CONTABLE"].str.strip().str.upper().eq(_TOTAL_LABEL)
    total_rows = df[is_total]
    data_rows = df[~is_total].copy()

    # Assign group label and sort key to each row
    data_rows, group_order = _assign_group_order(
    ...
    if sort_col and sort_col in data_rows.columns:
        data_rows = data_rows.sort_values(
            ["_group_order", sort_col], ascending=[True, False]
        )
    else:
        data_rows = data_rows.sort_values("_group_order")

    # Build one sentinel header DataFrame per group, then interleave with data
    blank_vals = {c: None for c in value_cols}
    group_changes = data_rows["_group"] != data_rows["_group"].shift()
    header_positions = data_rows[group_changes]

    sentinel_rows = pd.DataFrame([
        {"CUENTA_CONTABLE": _GROUP_SENTINEL, "DESCRIPCION": g, **blank_vals}
        for g in header_positions["_group"]
    ], columns=df.columns)
    # Cast value columns to float so concat doesn't warn about dtype promotion
    for _vc in value_cols:
        sentinel_rows[_vc] = sentinel_rows[_vc].astype(float)

    # Tag each sentinel with its insertion index so we can interleave correctly
    sentinel_rows.index = header_positions.index - 0.5  # type: ignore[assignment]

    data_clean = data_rows.drop(columns=["_group", "_group_order"])
    if sentinel_rows.empty:
        combined = data_clean.reset_index(drop=True)
    else:
        combined = pd.concat([data_clean, sentinel_rows]).sort_index().reset_index(drop=True)

    # Re-attach TOTAL row(s)
    if total_rows.empty:
        return combined[df.columns]
    return pd.concat([combined, total_rows], ignore_index=True)[df.columns]
...
def _pl_row_type(labels, raw_values):
    ...
    if is_blank:
        return "blank"
    if any(str(l).strip() == _TOTAL_LABEL for l in labels):
        return "total"
    first_label = str(labels[0]).strip() if labels else ""
    if first_label == _FINAL_TOTAL_LABEL:
        return "final_total"
    if first_label in PL_SUBTOTAL_LABELS:
        return "subtotal"
    return "normal"
...
def _bs_row_type(labels, raw_values):
    ...
    Partida rows are treated as normal so they participate in zebra striping.
    Section grand totals (TOTAL ACTIVO, TOTAL PASIVO Y PATRIMONIO) get
    final_total (top line + bold bottom line); other subtotals get total.
    """
    is_blank = all(_is_missing(v) for v in raw_values) and all(l == "" for l in labels)
    if is_blank:
        return "blank"
    first_label = str(labels[0]).strip() if labels else ""
    if first_label in _BS_FINAL_TOTAL_LABELS:
        return "final_total"
    if first_label in BS_SUBTOTAL_LABELS:
        return "total"
    return "normal"
...
def _df_to_rows(df, label_cols, value_cols, *, row_type_classifier=None):
    ...
    for record in df.to_dict("records"):
        labels = [
            "" if _is_missing(record[c]) else record[c]
            for c in label_cols
        ]
        raw_values = [record[c] for c in value_cols]
...
def _render_nit_pivot(pdf: FinancialPDF, nit_df, year_label, *,
    ...
    *nit_df* has columns: NIT, RAZON_SOCIAL, <account descriptions...>, TOTAL.
    ...
    keep_cols = [c for c in value_cols if c == _TOTAL_LABEL or nit_df[c].abs().sum() > 0.5]
    ...
    zero_mask = nit_df[value_cols].abs().sum(axis=1) <= 0.5
...
def export_to_pdf(output_path, data: PdfReportData):
    ...
    period_label = _build_period_label(data)
    ...
    # After cover, all subsequent pages get the header
    pdf.is_cover = False
    pdf.set_left_margin(PAGE_MARGIN)
    pdf.set_right_margin(PAGE_MARGIN)

    # ===== Page 3: Balance Sheet column names (needed early for _pdf_has_data) =====
    from pdf_reports import build_bs_column_names
    bs_col_names = list(build_bs_column_names(data.year))
    n_bs_vals = len(bs_col_names)

    # Pre-convert PL DataFrames to row dicts (needed for _pdf_has_data)
    pl_row_cache = {}
    _PL_ATTRS = [
    ...
    from nota_config import NOTA_GROUPS, numbered_groups, nota_title as _nota_title, RenderPattern, build_partida_nota_map
    _attr_label_cols = {
    ...
    # Build nota map: {PARTIDA label -> nota number string} based on which entries have data
    ...
    _render_table(pdf, bs_rows, ["", "Nota"], bs_col_names, bs_nota_widths, bs_val_w, alternating=True,
                  label_aligns=["L", "C"])
    ...
    for group, numbered_entries in numbered_groups(_pdf_has_data):
        if first_group:
            # Very first notes page: full 3-line header
            pdf.page_title = "Notas a los estados financieros"
            pdf.page_subtitle = subtitle_text
            pdf.show_company_in_header = True
            first_group = False
        pdf.add_page()
        # Any auto-breaks within this group also get continuation header
        ...
        for i, (num, entry) in enumerate(numbered_entries):
            ...
            title = _nota_title(num, entry.label)
            _render_subheader(pdf, title)

            if entry.is_bs:
                bs_df = data.bs_details[entry.bs_key]
                # Grouped BS sections: aggregate by prefix-group, single DESCRIPCION column
                if entry.bs_key in BS_GROUP_TABLES:
                    bs_df = _aggregate_by_group(bs_df, bs_col_names, BS_GROUP_TABLES[entry.bs_key])
                ...
                r = _drop_zero_rows(
                    _df_to_rows(bs_df, label_cols, bs_col_names)
                )
                ...
                if (entry.nit_ranking_key
                        and entry.nit_ranking_key in data.nit_rankings):
                    ...
                    if not nit_rank_df.empty:
                        ...
                        # Truncate RAZON_SOCIAL to fit within the label column
                        pdf.set_font("Helvetica", "", FONT_SIZE_DATA)
                        col_w = nit_rank_widths[0] - LABEL_INDENT
                        nit_rank_df = nit_rank_df.assign(
                            RAZON_SOCIAL=nit_rank_df["RAZON_SOCIAL"].apply(
                                lambda s: _truncate_text(pdf, s, col_w)
                                if isinstance(s, str) else s
                            )
                        )
                        nit_rows = _drop_zero_rows(
                            _df_to_rows(nit_rank_df,
                                        ["RAZON_SOCIAL"], bs_col_names)
                        )
                        if nit_rows:
                            _render_table(pdf, nit_rows,
                                          ["RAZON SOCIAL"], bs_col_names,
                                          nit_rank_widths, nit_rank_val_w,
                                          alternating=True)
