...
def _clean_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Shared column cleaning: copy, extract FIRST_CHAR, parse FECHA, extract MES.

    CUENTA_CONTABLE and CENTRO_COSTO are already trimmed in SQL via
    LTRIM(RTRIM(...)) in queries.py.
    """
...
def filter_for_statements(df: pd.DataFrame) -> pd.DataFrame:
    """Filter to statement-relevant accounts (prefix >= 61.9, excluding 79.1.1.1.01)."""
    prefix = pd.to_numeric(df["CUENTA_CONTABLE"].str[:4], errors="coerce")
    df = df[prefix >= 61.9]
...
def assign_partida_pl(df: pd.DataFrame) -> pd.DataFrame:
    ...
    cuenta4 = cuenta.str[:4]
    cuenta2 = cuenta.str[:2]
    ...
    df["PARTIDA_PL"] = pd.Categorical(np.select(conditions, choices, default="POR DEFINIR"))

    n_undefined = (df["PARTIDA_PL"] == "POR DEFINIR").sum()
    if n_undefined > 0:
        sample = df.loc[df["PARTIDA_PL"] == "POR DEFINIR", "CUENTA_CONTABLE"].unique()[:5]
        logger.warning(
            "%d rows assigned 'POR DEFINIR'. Sample CUENTA_CONTABLE: %s",
            n_undefined, list(sample),
        )
