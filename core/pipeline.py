...
from config.exceptions import ExportError, EmailError
from transforms.transforms import prepare_bs_stmt
from data.fetcher import fetch_all_data  # noqa: F401
from export.excel.builder import build_excel_data, build_bs_data, BS_DETAIL_SHEETS  # noqa: F401
from export.pdf.builder import build_pdf_data, BS_PDF_DETAIL_NOTES  # noqa: F401
...
def run_report(
    ...
    conn_factory=None, email_sender=None, no_email: bool = False,
    ...
    raw, raw_current_full, raw_prev, raw_bs, raw_bs_prev = fetch_all_data(
        company, year, month, conn_factory=conn_factory, need_pdf=not excel_only,
    )
    logger.info("Fetch data: %.2fs", time.perf_counter() - t0)
    ...
    # --- Email (non-fatal) ---
    attachments = [output] + ([pdf_output] if pdf_output else [])
    if no_email or not send_email:
        logger.info("Email step skipped.")
    else:
        if email_sender is None:
            email_sender = get_email_sender()
        try:
            email_sender.send(attachments, company=company, period=plabel, year=year)
        except (EmailError, OSError) as exc:
            logger.warning("Failed to send email — files were saved successfully: %s", exc)
